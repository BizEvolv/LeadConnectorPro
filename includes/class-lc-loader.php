 ...
 * @subpackage LeadConnectorPro/includes
 */
...
class LeadConnector_Loader
{
    ...
    public function run()
    {
        ...
        /**
         * Cron Fix: Clean up the legacy lc_twicedaily_refresh_req cron hook
         * if it is still scheduled. This prevents duplicate/stale cron jobs
         * left over from plugin versions prior to 3.0.10.4 which used the
         * old hook name. Runs once on init and self-clears after cleanup.
         */
        if ( ! function_exists( 'lc_cleanup_legacy_cron' ) ) {
            function lc_cleanup_legacy_cron() {
                if ( wp_next_scheduled( 'lc_twicedaily_refresh_req' ) ) {
                    global $wpdb;

                    // Remove all cron entries via direct DB update to handle
                    // large accumulated job queues that wp_clear_scheduled_hook
                    // may miss or time out on.
                    $wpdb->query( "UPDATE {$wpdb->options} SET option_value = 'a:0:{}' WHERE option_name = 'cron'" );

                    // Also remove with the standard WP API as a safety net.
                    wp_clear_scheduled_hook( 'lc_twicedaily_refresh_req' );

                    // Flush object-cache copies of the cron option.
                    wp_cache_delete( 'cron', 'options' );
                    wp_cache_delete( 'alloptions', 'options' );
                }
            }
        }
        add_action( 'init', 'lc_cleanup_legacy_cron' );

        /**
         * Admin Tools page: "LC Cron Fixer" under Tools menu.
         * Lets admins manually trigger the legacy cron cleanup if needed.
         */
        if ( ! function_exists( 'lc_cron_fixer_admin_menu' ) ) {
            function lc_cron_fixer_admin_menu() {
                add_submenu_page(
                    'tools.php',
                    'LeadConnector Pro Cron Fixer',
                    'LC Cron Fixer',
                    'manage_options',
                    'leadconnector-cron-fix',
                    'lc_cron_fixer_admin_page'
                );
            }
        }
        add_action( 'admin_menu', 'lc_cron_fixer_admin_menu' );

        if ( ! function_exists( 'lc_cron_fixer_admin_page' ) ) {
            function lc_cron_fixer_admin_page() {
                if ( isset( $_POST['lc_fix_cron'] ) && check_admin_referer( 'lc_fix_cron_action', 'lc_fix_cron_nonce' ) ) {
                    lc_cleanup_legacy_cron();
                    echo '<div class="notice notice-success"><p>LeadConnector Pro cron jobs have been cleared successfully.</p></div>';
                }
                $legacy_scheduled = wp_next_scheduled( 'lc_twicedaily_refresh_req' );
                ?>
                <div class="wrap">
                    <h1>LeadConnector Pro Cron Fixer</h1>
                    <?php if ( $legacy_scheduled ) : ?>
                        <div class="notice notice-warning">
                            <p>Found legacy <code>lc_twicedaily_refresh_req</code> cron job (next run: <?php echo esc_html( date( 'Y-m-d H:i:s', $legacy_scheduled ) ); ?>). Click the button below to remove it.</p>
                        </div>
                        <form method="post">
                            <?php wp_nonce_field( 'lc_fix_cron_action', 'lc_fix_cron_nonce' ); ?>
                            <button type="submit" name="lc_fix_cron" class="button button-primary">Fix Cron Jobs</button>
                        </form>
                    <?php else : ?>
                        <div class="notice notice-success">
                            <p>No legacy cron jobs found. Your LeadConnector Pro cron schedule is healthy.</p>
                        </div>
                    <?php endif; ?>
                </div>
                <?php
        ...
        function schedule_oauth_refresh_cron(){
            ...
            if ( ! wp_next_scheduled ( 'lcpro_twicedaily_refresh_req' ) ) {
                wp_schedule_event( time(), 'twicedaily', 'lcpro_twicedaily_refresh_req' );
            }
        ...
        add_action( 'lcpro_twicedaily_refresh_req', $oauth_refresh_schedule_hook );
