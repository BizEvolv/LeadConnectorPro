 ...
 * @subpackage LeadConnectorPro/admin
 */
...
class LeadConnector_Admin
{
    ...
    public function lc_public_api_proxy($request)
    {
        ...
        if ($endpoint == "wp_validate_auth_state") {
            ...
            // Security: Never return raw tokens, api keys, or full options in the response.
            // Only return the minimal connection status fields the admin UI needs.
            return array(
        ...
        if ($endpoint == "wp_delete_post") {
            // Security: wp_delete_post must only be called via POST, never GET.
            if ($request->get_method() !== 'POST') {
            ...
            // Security: Verify the post belongs to the LC funnels custom post type.
            // This prevents deletion of arbitrary WordPress posts (CVE-2024-1371 class fix).
            $post = get_post($post_id);
            if (!$post || $post->post_type !== lead_connector_constants\lc_custom_post_type) {
                return new WP_Error('forbidden', 'Post not found or not deletable via this endpoint.', array('status' => 403));
            }
        ...
        if ($directEndpoint == "true") {
            // Security: Restrict outbound requests to approved LeadConnector Pro domains only.
            // Allowing arbitrary URLs is a Server-Side Request Forgery (SSRF) risk.
            $allowed_hosts = array(
            ...
            if (!in_array($host, $allowed_hosts, true)) {
                return new WP_Error('forbidden_host', 'External requests are only allowed to approved LeadConnector Pro domains.', array('status' => 403));
            }
            $args = array(
                'timeout' => 60,
            );
    ...
    public function enqueue_styles($hook)
    {
        ...
        if ($hook != 'toplevel_page_lcpro-plugin') {
            return;
        }
    ...
    public function enqueue_scripts($hook)
    {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'proxy_url' => rest_url('lcpro_api/v1/proxy'),
    ...
    {
        // add_settings_section('api_settings_inputs', __('leadconnector-pro', 'LeadConnector Pro'), 'lead_connector_section_text1', 'lead_connector_plugin');
    ...
    {
        // REST namespace rebranded to lcpro_api to match LeadConnector Pro.
        // Both routes require manage_options capability (CVE-2026-1890 / CVE-2024-1371 fix).
        register_rest_route('lcpro_api/v1', '/proxy', array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => array($this, 'lc_public_api_proxy'),
            'permission_callback' => function (WP_REST_Request $request) {
                // Verify the WP REST nonce AND require manage_options capability.
                // This closes CVE-2026-1890: unauthenticated callers cannot reach this route.
